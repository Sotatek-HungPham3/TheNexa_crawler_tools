...
try:
    ...
    sql = "SELECT symbol, ref_id FROM masterdata WHERE exchange ='coinone'"
    ...
    sql = "SELECT id, in_token, out_token, fee_token FROM coinone_summary_transactions"
    mycursor.execute(sql)
    ...
    for row in rows:
        ...
        sql = """
            UPDATE coinone_summary_transactions
